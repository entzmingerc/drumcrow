...
function setup_input()
	...
	-- use input[1].stream to update the synths constantly?
    input[1].stream = function (v)
        if v < 0 then
            v = v / 4.9514 * 5
        else
            --v = v / 5.048 * 5
			-- map 0-10V to -10V - +10V? "almost 10/10"
			v = (v - 5) * 1.95
        end
		
		-- here is where we define what c2 does 
...
function setup_i2c()
	...
	-- CROW.C1 x   -- set command and channel
	...
	-- CROW.C2 x y -- set sound engine parameter
